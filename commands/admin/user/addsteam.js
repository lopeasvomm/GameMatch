...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    statements.insertSteamAccount.run(user.id, info.steamID64);
    ...
    const insertGames = db.transaction((games) => {
      ...
      for (const game of games) {
        ...
        if (foundGame && !userGames.includes(foundGame.id)) {
          statements.insertUserGame.run(user.id, foundGame.id);
          i++;
        }
      }
      return i;
    ...
    return interaction.followUp({
      content: `Successfully added ${formatSteamAccount(info)} to ${
        user.tag
      }'s account.\nAdded \`${i}\` games.`,
    });
  },
